...
@[spec_gen_rv64] theorem sltu_spec_gen_rd_eq_rs2 (rd rs1 : Reg) (v1 v2 : Word)
    ...
    cpsTriple addr (addr + 4)
      ((addr ↦ᵢ .SLTU rd rs1 rd) ** (rs1 ↦ᵣ v1) ** (rd ↦ᵣ v2))
      ((addr ↦ᵢ .SLTU rd rs1 rd) ** (rs1 ↦ᵣ v1) ** (rd ↦ᵣ (if BitVec.ult v1 v2 then (1 : Word) else 0))) :=
...
@[spec_gen_rv64] theorem mul_spec_gen (rd rs1 rs2 : Reg) (v_old v1 v2 : Word)
    (addr : Addr) (hrd_ne_x0 : rd ≠ .x0) :
    cpsTriple addr (addr + 4)
      ((addr ↦ᵢ .MUL rd rs1 rs2) ** (rs1 ↦ᵣ v1) ** (rs2 ↦ᵣ v2) ** (rd ↦ᵣ v_old))
      ((addr ↦ᵢ .MUL rd rs1 rs2) ** (rs1 ↦ᵣ v1) ** (rs2 ↦ᵣ v2) ** (rd ↦ᵣ (v1 * v2))) :=
...
@[spec_gen_rv64] theorem mul_spec_gen_rd_eq_rs1 (rd rs2 : Reg) (v1 v2 : Word)
    (addr : Addr) (hrd_ne_x0 : rd ≠ .x0) (hne : rd ≠ rs2) :
    cpsTriple addr (addr + 4)
      ((addr ↦ᵢ .MUL rd rd rs2) ** (rd ↦ᵣ v1) ** (rs2 ↦ᵣ v2))
      ((addr ↦ᵢ .MUL rd rd rs2) ** (rd ↦ᵣ (v1 * v2)) ** (rs2 ↦ᵣ v2)) :=
...
@[spec_gen_rv64] theorem mulhu_spec_gen (rd rs1 rs2 : Reg) (v_old v1 v2 : Word)
    (addr : Addr) (hrd_ne_x0 : rd ≠ .x0) :
    cpsTriple addr (addr + 4)
      ((addr ↦ᵢ .MULHU rd rs1 rs2) ** (rs1 ↦ᵣ v1) ** (rs2 ↦ᵣ v2) ** (rd ↦ᵣ v_old))
      ((addr ↦ᵢ .MULHU rd rs1 rs2) ** (rs1 ↦ᵣ v1) ** (rs2 ↦ᵣ v2) ** (rd ↦ᵣ rv64_mulhu v1 v2)) :=
