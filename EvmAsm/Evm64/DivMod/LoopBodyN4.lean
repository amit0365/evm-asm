...
/-- Full loop body (BLTU ntaken + BEQ skip) for n=4.
    No overlapping cells: u_hi=uTop, u_lo=u3, v_top=v3.
...
theorem divK_loop_body_n4_max_skip_spec
  ...
  -- 1. Trial max full (base+448 → base+516), instantiated with n=4, u_hi=uTop, u_lo=u3, v_top=v3
  ...
  have MCS := divK_mulsub_correction_skip_spec sp q_hat j v0 v1 v2 v3 u0 u1 u2 u3 uTop
    j u3 vtop_base uTop v3 v2_old base
  ...
  --    (u_base+4064 ↦ₘ uTop, u_base+4072 ↦ₘ u3, sp+56 ↦ₘ v3 are already in TF)
...
theorem divK_loop_body_n4_call_skip_spec
    ...
    let q1 := rv64_divu uTop d_hi
    let rhat := uTop - q1 * d_hi
