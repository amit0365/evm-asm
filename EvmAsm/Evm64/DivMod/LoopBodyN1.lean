...
theorem divK_loop_body_n1_max_skip_spec
    ...
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      (loopBodyPre (1 : Word) sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
        v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old)
      (base + loopBodyOff)
      (loopBodyN1SkipPost sp j (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 u_top)
      (base + denormOff)
      (loopBodyN1SkipPost sp j (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  -- Re-introduce addresses and q_hat as local lets for the proof body
  let u_base := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
...
theorem divK_loop_body_n1_max_addback_spec
    ...
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      ...
      (base + denormOff)
      (loopBodyN1AddbackBeqPost sp j (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  -- Re-introduce addresses and derived values as local lets for the proof body
  let u_base := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
  let q_hat : Word := signExtend12 4095
  let q_addr := sp + signExtend12 4088 - j <<< (3 : BitVec 6).toNat
  let ms := mulsubN4 q_hat v0 v1 v2 v3 u0 u1 u2 u3
...
theorem divK_loop_body_n1_call_skip_spec
    ...
    (hborrow :
      (if BitVec.ult u_top
           (mulsubN4_c3 (div128Quot u1 u0 v0) v0 v1 v2 v3 u0 u1 u2 u3)
       then (1 : Word) else 0) = (0 : Word)) :
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      (loopBodyPreWithScratch (1 : Word) sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
        v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old
        ret_mem d_mem dlo_mem scratch_un0)
      (base + loopBodyOff)
      (loopBodyN1CallSkipPostJ sp base j v0 v1 v2 v3 u0 u1 u2 u3 u_top)
      (base + denormOff)
      (loopBodyN1CallSkipPostJ sp base j v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  -- Re-introduce addresses and div128 intermediates as local lets for the proof body
  let u_base := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
  let d_hi := v0 >>> (32 : BitVec 6).toNat
  let d_lo := (v0 <<< (32 : BitVec 6).toNat) >>> (32 : BitVec 6).toNat
  let div_un1 := u0 >>> (32 : BitVec 6).toNat
...
theorem divK_loop_body_n1_call_addback_spec
    ...
    (hbltu : BitVec.ult u1 v0)
    (hcarry2_nz :
      let ms := mulsubN4 (div128Quot u1 u0 v0) v0 v1 v2 v3 u0 u1 u2 u3
      let ab := addbackN4 ms.1 ms.2.1 ms.2.2.1 ms.2.2.2.1 (u_top - ms.2.2.2.2) v0 v1 v2 v3
      addbackN4_carry ms.1 ms.2.1 ms.2.2.1 ms.2.2.2.1 v0 v1 v2 v3 = 0 →
      addbackN4_carry ab.1 ab.2.1 ab.2.2.1 ab.2.2.2.1 v0 v1 v2 v3 ≠ 0)
    (hborrow :
    ...
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      ...
      (base + denormOff)
      (loopBodyN1CallAddbackBeqPostJ sp base j v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  -- Re-introduce addresses and div128 intermediates as local lets for the proof body
  let u_base := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
  let d_hi := v0 >>> (32 : BitVec 6).toNat
  let d_lo := (v0 <<< (32 : BitVec 6).toNat) >>> (32 : BitVec 6).toNat
...
/-- Unified loop body (BLTU ntaken) for n=1, parameterized by borrow condition.
...
theorem divK_loop_body_n1_max_unified_spec
    (borrow_zero : Bool)
    (sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
     v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old : Word)
    (base : Word)
    ...
    (hborrow : if borrow_zero
               then (if BitVec.ult u_top
                        (mulsubN4_c3 (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3)
                     then (1 : Word) else 0) = (0 : Word)
               else (if BitVec.ult u_top
                        (mulsubN4_c3 (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3)
                     then (1 : Word) else 0) ≠ (0 : Word)) :
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      (loopBodyPre (1 : Word) sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
        v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old)
      (base + loopBodyOff)
      (loopBodyUnifiedPostN1 borrow_zero sp j (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 u_top)
      (base + denormOff)
      (loopBodyUnifiedPostN1 borrow_zero sp j (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  cases borrow_zero
...
/-- Unified loop body (BLTU taken, call path) for n=1, parameterized by borrow condition. -/
theorem divK_loop_body_n1_call_unified_spec
    (borrow_zero : Bool)
    (sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
     v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old : Word)
    (ret_mem d_mem dlo_mem scratch_un0 : Word)
    ...
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      (loopBodyPreWithScratch (1 : Word) sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
        ...
        ret_mem d_mem dlo_mem scratch_un0)
      (base + loopBodyOff)
      (loopBodyUnifiedPostN1 borrow_zero sp j (div128Quot u1 u0 v0) v0 v1 v2 v3 u0 u1 u2 u3 u_top **
       (sp + signExtend12 3968 ↦ₘ (base + 516)) **
       (sp + signExtend12 3960 ↦ₘ v0) **
      ...
      (loopBodyUnifiedPostN1 borrow_zero sp j (div128Quot u1 u0 v0) v0 v1 v2 v3 u0 u1 u2 u3 u_top **
       ...
       (sp + signExtend12 3960 ↦ₘ v0) **
       (sp + signExtend12 3952 ↦ₘ div128DLo v0) **
       (sp + signExtend12 3944 ↦ₘ div128Un0 u0)) := by
  cases borrow_zero
  · -- false (addback+BEQ path)
    rw [if_neg (by decide)] at hborrow
    simp only [loopBodyUnifiedPostN1, loopBodyUnifiedPost_false]
