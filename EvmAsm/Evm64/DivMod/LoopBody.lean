...
theorem divK_mulsub_full_spec
  ...
  have S := divK_mulsub_setup_spec sp q_hat j v1Old v5Old v6Old v10Old (base + 516)
  rw [lb_ms_setup] at S
...
theorem divK_correction_skip_spec
    ...
    cpsTriple (base + 728) (base + 884) (sharedDivModCode base)
      ((.x12 ↦ᵣ sp) ** (.x6 ↦ᵣ u_base) ** (.x7 ↦ᵣ (0 : Word)) **
       (.x11 ↦ᵣ q_hat) ** (.x5 ↦ᵣ v5Old) ** (.x2 ↦ᵣ v2Old) ** (.x0 ↦ᵣ (0 : Word)) **
       ((sp + signExtend12 32) ↦ₘ v0) ** ((u_base + signExtend12 0) ↦ₘ u0) **
       ((sp + signExtend12 40) ↦ₘ v1) ** ((u_base + signExtend12 4088) ↦ₘ u1) **
       ((sp + signExtend12 48) ↦ₘ v2) ** ((u_base + signExtend12 4080) ↦ₘ u2) **
       ((sp + signExtend12 56) ↦ₘ v3) ** ((u_base + signExtend12 4072) ↦ₘ u3) **
  ...
  have skip_framed := cpsTriple_frameR
    ((.x12 ↦ᵣ sp) ** (.x6 ↦ᵣ u_base) **
     (.x11 ↦ᵣ q_hat) ** (.x5 ↦ᵣ v5Old) ** (.x2 ↦ᵣ v2Old) **
     ((sp + signExtend12 32) ↦ₘ v0) ** ((u_base + signExtend12 0) ↦ₘ u0) **
     ((sp + signExtend12 40) ↦ₘ v1) ** ((u_base + signExtend12 4088) ↦ₘ u1) **
     ((sp + signExtend12 48) ↦ₘ v2) ** ((u_base + signExtend12 4080) ↦ₘ u2) **
     ((sp + signExtend12 56) ↦ₘ v3) ** ((u_base + signExtend12 4072) ↦ₘ u3) **
...
theorem divK_save_trial_load_spec
    (sp j n jOld v5Old v6Old v7Old v10Old u_hi u_lo v_top : Word)
    (base : Word) :
  ...
  have TL := divK_trial_load_spec sp j n v5Old v6Old v7Old v10Old u_hi u_lo v_top
    (base + 452)
  dsimp only [] at TL
...
theorem divK_trial_call_path_spec
  ...
  have D := div128_spec sp (base + 516) v_top u_lo u_hi base
    j vtop_base v11Old ret_mem d_mem dlo_mem un0_mem
    halign
  dsimp only [] at D
...
theorem divK_store_loop_spec
    (sp j q_hat v5Old v7Old qOld : Word)
    (base : Word) :
  ...
  have SQ := divK_store_qj_spec sp j q_hat v5Old v7Old qOld (base + 884)
  dsimp only [] at SQ
  rw [lb_sqj] at SQ
  ...
  have SQx0 : cpsTriple (base + 884) (base + 900) (sharedDivModCode base)
      ((.x1 ↦ᵣ j) ** (.x12 ↦ᵣ sp) ** (.x11 ↦ᵣ q_hat) **
       (.x5 ↦ᵣ v5Old) ** (.x7 ↦ᵣ v7Old) ** (.x0 ↦ᵣ (0 : Word)) ** (q_addr ↦ₘ qOld))
      ((.x1 ↦ᵣ j) ** (.x12 ↦ᵣ sp) ** (.x11 ↦ᵣ q_hat) **
...
theorem divK_store_loop_j0_spec
    (sp q_hat v5Old v7Old qOld : Word)
    (base : Word) :
  ...
  intro q_addr j'
  -- 1. Store q[j]: instrs [109]-[112] at base+884
  have SQ := divK_store_qj_spec sp (0 : Word) q_hat v5Old v7Old qOld (base + 884)
  dsimp only [] at SQ
  rw [lb_sqj] at SQ
  have SQe := cpsTriple_extend_code (hmono := by
    exact CodeReq.union_sub (lb_sub base 109 _ _ (by decide) (by bv_addr) (by decide))
...
theorem divK_trial_max_full_spec
    (sp j n jOld v5Old v6Old v7Old v10Old v11Old u_hi u_lo v_top : Word)
    (base : Word)
  ...
  have STL := divK_save_trial_load_spec sp j n jOld v5Old v6Old v7Old v10Old u_hi u_lo v_top
    base
  dsimp only [] at STL
...
theorem divK_trial_call_full_spec
    (sp j n jOld v5Old v6Old v7Old v10Old v11Old v2Old u_hi u_lo v_top : Word)
    (ret_mem d_mem dlo_mem un0_mem : Word)
    (base : Word)
    (halign : ((base + 516) + signExtend12 (0 : BitVec 12)) &&& ~~~(1 : Word) = base + 516)
    (hbltu : BitVec.ult u_hi v_top) :
  ...
  have TCP := divK_trial_call_path_spec sp j u_lo u_hi v_top vtop_base base
    v2Old v11Old ret_mem d_mem dlo_mem un0_mem
    halign
  dsimp only [] at TCP
