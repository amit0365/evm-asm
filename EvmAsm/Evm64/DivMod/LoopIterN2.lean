...
-- j=2 max+addback, call+skip, call+addback follow the same pattern but are omitted
-- for now to keep the file manageable. They will be added as needed for full path composition.
