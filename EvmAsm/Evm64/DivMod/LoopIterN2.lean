...
theorem divK_loop_body_n2_max_addback_j0_spec
  ...
  exact cpsTriple_consequence _ _ _ _ _ _ _
    (fun h hp => by xperm_hyp hp)
    (fun h hp => by delta loopBodyN2AddbackPost loopBodyAddbackPost mulsubN4 addbackN4 loopExitPostN2 loopExitPost; rw [sepConj_assoc'] at hp; xperm_hyp hp)
    full
  ...
  exact cpsTriple_consequence _ _ _ _ _ _ _
    ...
    (fun h hp => by
      delta loopBodyN2CallAddbackPostJ div128Quot div128DLo div128Un0
            loopBodyN2AddbackPost loopBodyAddbackPost mulsubN4 addbackN4 loopExitPostN2 loopExitPost
      rw [sepConj_assoc'] at hp; xperm_hyp hp)
    full
...
set_option maxRecDepth 4096 in
set_option maxHeartbeats 6400000 in
/-- Loop body cpsTriple for n=2, call+addback, j=2.
    Since j=2, the BGE loop-back is taken (j' = 1 ≥ 0), giving a cpsTriple to base+448. -/
theorem divK_loop_body_n2_call_addback_j2_spec
    (sp j_old v5_old v6_old v7_old v10_old v11_old v2_old
     v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old : Word)
    (ret_mem d_mem dlo_mem scratch_un0 : Word)
    (base : Word)
    ...
    cpsTriple (base + 448) (base + 448) (sharedDivModCode base)
      ((.x12 ↦ᵣ sp) ** (.x1 ↦ᵣ (2 : Word)) **
       ...
       (sp + signExtend12 3952 ↦ₘ dlo_mem) **
       (sp + signExtend12 3944 ↦ₘ scratch_un0))
      (loopBodyN2CallAddbackPostJ sp base (2 : Word) v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  intro u_base q_addr
  let d_hi := v1 >>> (32 : BitVec 6).toNat
  let d_lo := (v1 <<< (32 : BitVec 6).toNat) >>> (32 : BitVec 6).toNat
  let div_un1 := u1 >>> (32 : BitVec 6).toNat
