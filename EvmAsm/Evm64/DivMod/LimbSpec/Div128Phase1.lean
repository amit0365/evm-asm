/-
  ...
  quotient halves):
    * `divK_div128_save_split_d_spec` — 6-instruction block (SD, SD,
      ...
      scratch, and splits `d` into `d_hi` / `d_lo`.
    ...
    * `divK_div128_step1_init_spec` — 3-instruction block (DIVU, MUL,
      SUB) computing `q1 = u_hi / d_hi` and `rhat = u_hi - q1 * d_hi`.
...
theorem divK_div128_step1_init_spec (u_hi d_hi v5Old v10Old : Word) (base : Word) :
  ...
  intro q1 rhat cr
  have I0 := divu_spec_gen .x10 .x7 .x6 v10Old u_hi d_hi base (by nofun)
  have I1 := mul_spec_gen .x5 .x10 .x6 v5Old q1 d_hi (base + 4) (by nofun)
