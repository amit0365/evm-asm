...
theorem divK_mulsub_limb_spec
    (sp u_base qHat carry_in v5Old v7Old v2Old v_i u_i : Word)
    ...
    cpsTriple base (base + 44) cr
      ((.x12 ↦ᵣ sp) ** (.x11 ↦ᵣ qHat) ** (.x10 ↦ᵣ carry_in) **
       (.x6 ↦ᵣ u_base) ** (.x5 ↦ᵣ v5Old) ** (.x7 ↦ᵣ v7Old) **
       (.x2 ↦ᵣ v2Old) **
  ...
  have I6 := ld_spec_gen .x2 .x6 u_base v2Old u_i u_off (base + 24) (by nofun)
...
theorem divK_addback_limb_spec
    (sp u_base carry_in v5Old v2Old v_i u_i : Word)
    (v_off u_off : BitVec 12) (base : Word) :
  ...
  have I0 := ld_spec_gen .x5 .x12 sp v5Old v_i v_off base (by nofun)
  have I1 := ld_spec_gen .x2 .x6 u_base v2Old u_i u_off (base + 4) (by nofun)
