...
theorem divK_addback_final_spec (u_base carry q_hat v5_old uTop : Word)
    ...
    cpsTriple base (base + 16) cr
      ((.x6 ↦ᵣ u_base) ** (.x7 ↦ᵣ carry) ** (.x11 ↦ᵣ q_hat) **
       (.x5 ↦ᵣ v5_old) ** (u_base + signExtend12 u_off ↦ₘ uTop))
      ((.x6 ↦ᵣ u_base) ** (.x7 ↦ᵣ carry) ** (.x11 ↦ᵣ q_hat') **
  ...
  have I2 := sd_spec_gen .x6 .x5 u_base uNew uTop u_off (base + 8)
