...
theorem divK_div128_clamp_test_q1_spec (q1 v5Old : Word) (base : Word) :
...
theorem divK_div128_clamp_test_q0_spec (q0 v1Old : Word) (base : Word) :
...
theorem divK_div128_step2_init_spec (un21 d_hi v1Old v5Old v11Old : Word) (base : Word) :
  ...
  intro q0 rhat2 cr
  have I0 := divu_spec_gen .x5 .x7 .x6 v5Old un21 d_hi base (by nofun)
  have I1 := mul_spec_gen .x1 .x5 .x6 v1Old q0 d_hi (base + 4) (by nofun)
  have I2 := sub_spec_gen .x11 .x7 .x1 un21 (q0 * d_hi) v11Old (base + 8) (by nofun)
  runBlock I0 I1 I2

/-- div128 product check 2: compute q0*d_lo and rhat2*2^32+un0 for comparison. -/
theorem divK_div128_prodcheck2_body_spec (sp q0 rhat2 v1Old v7Old dlo un0 : Word)
    (base : Word) :
    let q0_dlo := q0 * dlo
...
theorem divK_div128_combine_q_spec (q1 q0 v11Old : Word) (base : Word) :
...
theorem divK_div128_restore_return_spec (sp v2Old ret_addr : Word) (base : Word)
    ...
    cpsTriple base ret_addr cr
      ((.x12 ↦ᵣ sp) ** (.x2 ↦ᵣ v2Old) ** (sp + signExtend12 3968 ↦ₘ ret_addr))
      ((.x12 ↦ᵣ sp) ** (.x2 ↦ᵣ ret_addr) ** (sp + signExtend12 3968 ↦ₘ ret_addr)) := by
  intro cr
  have I0 := ld_spec_gen .x2 .x12 sp v2Old ret_addr 3968 base (by nofun)
