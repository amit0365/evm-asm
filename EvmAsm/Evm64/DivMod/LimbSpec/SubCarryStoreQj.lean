...
theorem divK_sub_carry_spec (u_base carry_in v5_old v7_old uTop : Word)
    (u_off : BitVec 12) (base : Word) :
    let borrow := if BitVec.ult uTop carry_in then (1 : Word) else 0
    let uNew := uTop - carry_in
    let cr :=
    ...
    cpsTriple base (base + 16) cr
      ((.x6 ↦ᵣ u_base) ** (.x10 ↦ᵣ carry_in) **
       ...
       ((u_base + signExtend12 u_off) ↦ₘ uTop))
      ((.x6 ↦ᵣ u_base) ** (.x10 ↦ᵣ carry_in) **
       (.x5 ↦ᵣ uNew) ** (.x7 ↦ᵣ borrow) **
       ((u_base + signExtend12 u_off) ↦ₘ uNew)) := by
  intro borrow uNew cr
  have I0 := ld_spec_gen .x5 .x6 u_base v5_old uTop u_off base (by nofun)
  have I1 := sltu_spec_gen .x7 .x5 .x10 v7_old uTop carry_in (base + 4) (by nofun)
  have I2 := sub_spec_gen_rd_eq_rs1 .x5 .x10 uTop carry_in (base + 8) (by nofun)
  have I3 := sd_spec_gen .x6 .x5 u_base uNew uTop u_off (base + 12)
