/-
  EvmAsm.Evm64.DivMod.Compose.ModN1FullShift0
...
-- ============================================================================
-- Address simplification lemmas for j=0 (n=1, shift=0)
-- ============================================================================

/-- u_base for j=0: sp + signExtend12 4056 - 0<<<3 = sp + signExtend12 4056 -/
private theorem j0_u_base_eq_s0n1 (sp : Word) :
...
private theorem word_add_zero_s0n1 (x : Word) : x + (0 : Word) = x := by bv_omega

/-- q_addr for j=0: sp + signExtend12 4088 - 0<<<3 = sp + signExtend12 4088 -/
private theorem j0_q_addr_eq_s0n1 (sp : Word) :
...
private theorem j0_u0_addr_eq_s0n1 (sp : Word) :
...
private theorem j0_u1_addr_eq_s0n1 (sp : Word) :
...
private theorem j0_u2_addr_eq_s0n1 (sp : Word) :
...
private theorem j0_u3_addr_eq_s0n1 (sp : Word) :
...
private theorem j0_u4_addr_eq_s0n1 (sp : Word) :
...
private theorem j0_uhi_addr_eq_n1_s0 (sp : Word) :
...
private theorem j0_ulo_addr_eq_n1_s0 (sp : Word) :
...
private theorem j0_vtop_addr_eq_n1_s0 (sp : Word) :
...
private theorem j1_q_addr_eq_s0n1 (sp : Word) :
...
private theorem j1_u0_addr_eq_s0n1 (sp : Word) :
...
private theorem j1_u1_addr_eq_s0n1 (sp : Word) :
...
private theorem j1_u2_addr_eq_s0n1 (sp : Word) :
...
private theorem j1_u3_addr_eq_s0n1 (sp : Word) :
...
private theorem j1_u4_addr_eq_s0n1 (sp : Word) :
...
private theorem j1_uhi_addr_eq_n1_s0 (sp : Word) :
...
private theorem j1_ulo_addr_eq_n1_s0 (sp : Word) :
...
private theorem j1_sub_8_s0n1 (sp : Word) :
...
private theorem j1_q_sub_8_s0n1 (sp : Word) :
...
-- ============================================================================
-- Address simplification lemmas for j=2 (n=1, shift=0)
-- ============================================================================

/-- u_base for j=2: sp + SE12(4056) - 2<<<3 = sp + SE12(4040) -/
private theorem j2_u_base_eq_s0n1 (sp : Word) :
...
private theorem j2_q_addr_eq_s0n1 (sp : Word) :
...
private theorem j2_u0_addr_eq_s0n1 (sp : Word) :
...
private theorem j2_u1_addr_eq_s0n1 (sp : Word) :
...
private theorem j2_u2_addr_eq_s0n1 (sp : Word) :
...
private theorem j2_u3_addr_eq_s0n1 (sp : Word) :
...
private theorem j2_u4_addr_eq_s0n1 (sp : Word) :
...
private theorem j2_uhi_addr_eq_n1_s0 (sp : Word) :
...
private theorem j2_ulo_addr_eq_n1_s0 (sp : Word) :
...
private theorem j2_q_sub_16_s0n1 (sp : Word) :
...
-- ============================================================================
-- Address simplification lemmas for j=3 (n=1, shift=0)
-- ============================================================================

/-- u_base for j=3: sp + SE12(4056) - 3<<<3 = sp + SE12(4032) -/
private theorem j3_u_base_eq_s0n1 (sp : Word) :
...
private theorem j3_q_addr_eq_s0n1 (sp : Word) :
...
private theorem j3_u0_addr_eq_s0n1 (sp : Word) :
...
private theorem j3_u1_addr_eq_s0n1 (sp : Word) :
...
private theorem j3_u2_addr_eq_s0n1 (sp : Word) :
...
private theorem j3_u3_addr_eq_s0n1 (sp : Word) :
...
private theorem j3_u4_addr_eq_s0n1 (sp : Word) :
...
private theorem j3_uhi_addr_eq_n1_s0 (sp : Word) :
...
private theorem j3_ulo_addr_eq_n1_s0 (sp : Word) :
...
private theorem j3_q_sub_24_s0n1 (sp : Word) :
...
private theorem se12_4_sub_1_s0n1 :
...
private theorem cpsBranch_loop_back_then_cpsTriple_s0n1
  ...
  · -- Not taken: exited to exit_. Weaken Q_exit → Q.
    ...
    obtain ⟨h_full, hcompat, h_q, h_f, hdisj, heq, hQex, hFv⟩ := hQexitF
    exact ⟨h_full, hcompat, h_q, h_f, hdisj, heq, h_exit _ hQex, hFv⟩
...
private theorem cpsTriple_seq_ex_same_cr_s0n1 {V : Type} (s m e : Word) (cr : CodeReq)
...
/-- Pre-loop + four-iteration loop body for n=1 MOD (shift=0).
    Composes base→base+448 (copy path, no normalization) with four iterations of loop body
...
theorem evm_mod_n1_shift0_preloop_loopbody_spec (sp base : Word)
  ...
  have hPL := evm_mod_n1_shift0_to_loopSetup_spec sp base a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10
    q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7 n_mem shift_mem
    hbnz hb3z hb2z hb1z hshift_z hvalid hv_q0 hv_q1 hv_q2 hv_q3
  ...
  rw [se12_4_sub_1_s0n1] at hPLf
  -- Step 2: j=3 cpsBranch (base+448 → base+448/904)
  -- For shift=0: b values unchanged (b0..b3), u values = copies of a
  -- The j=3 window positions:
  -- u[0] at SE12(4032) = a3, u[1] at SE12(4024) = 0,
  -- u[2] at SE12(4016) = 0, u[3] at SE12(4008) = 0,
  -- u_top at SE12(4000) = 0, q_old at SE12(4064) = 0
  have hJ3 := divK_loop_body_n1_combined_spec
    sp (3 : Word) j_old (1 : Word) (clzResult b0).1 ((clzResult b0).2 >>> (63 : Nat)) b3 v11
    (signExtend12 (0 : BitVec 12) - (clzResult b0).1)
    b0 b1 b2 b3 a3 (0 : Word) (0 : Word) (0 : Word) (0 : Word) (0 : Word)
    ret_mem d_mem dlo_mem scratch_un0
    base
    hv_j hv_n
    (by rw [j3_uhi_addr_eq_n1_s0]; exact hv_u4)
    (by rw [j3_ulo_addr_eq_n1_s0]; exact hv_u3)
    (by rw [j0_vtop_addr_eq_n1_s0]; exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
  ...
  · -- Loop-back case: j=3 looped back to base+448, now execute j=2 then j=1 then j=0
    ...
    have hJ2 := divK_loop_body_n1_combined_spec
      ...
      b0 b1 b2 b3 a2 un0v_j3 un1v_j3 un2v_j3 un3v_j3 (0 : Word)
      retv_j3 dv_j3 dlov_j3 sunv_j3
      base
      hv_j hv_n
      (by rw [j2_uhi_addr_eq_n1_s0]; exact hv_u3)
      (by rw [j2_ulo_addr_eq_n1_s0]; exact hv_u2)
      (by rw [j0_vtop_addr_eq_n1_s0]; exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
      hv_ret hv_d hv_dlo hv_scratch
      halign
      (by rw [show signExtend12 (32 : BitVec 12) = (32 : Word) from by native_decide]
          exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
      (by rw [j2_u0_addr_eq_s0n1]; exact hv_u2)
      (by rw [show signExtend12 (40 : BitVec 12) = (40 : Word) from by native_decide]
          exact ValidMemRange.fetch hvalid 5 _ (by omega) (by rfl))
      (by rw [j2_u1_addr_eq_s0n1]; exact hv_u3)
      (by rw [show signExtend12 (48 : BitVec 12) = (48 : Word) from by native_decide]
          exact ValidMemRange.fetch hvalid 6 _ (by omega) (by rfl))
      (by rw [j2_u2_addr_eq_s0n1]; exact hv_u4)
      (by rw [show signExtend12 (56 : BitVec 12) = (56 : Word) from by native_decide]
          exact ValidMemRange.fetch hvalid 7 _ (by omega) (by rfl))
      (by rw [j2_u3_addr_eq_s0n1]; exact hv_u5)
      (by rw [j2_u4_addr_eq_s0n1]; exact hv_u6)
      (by rw [j2_q_addr_eq_s0n1]; exact hv_q2)
    dsimp only [] at hJ2
    simp only [j2_u0_addr_eq_s0n1, j2_q_addr_eq_s0n1, j2_u1_addr_eq_s0n1,
      j2_u2_addr_eq_s0n1, j2_u3_addr_eq_s0n1, j2_u4_addr_eq_s0n1,
    ...
    · -- j=2 loop-back: now execute j=1 then j=0
      ...
      have hJ1 := divK_loop_body_n1_combined_spec
        ...
        base
        hv_j hv_n
        (by rw [j1_uhi_addr_eq_n1_s0]; exact hv_u2)
        (by rw [j1_ulo_addr_eq_n1_s0]; exact hv_u1)
        (by rw [j0_vtop_addr_eq_n1_s0]; exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
        hv_ret hv_d hv_dlo hv_scratch
        halign
        (by rw [show signExtend12 (32 : BitVec 12) = (32 : Word) from by native_decide]
            exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
        (by rw [j1_u0_addr_eq_s0n1]; exact hv_u1)
        (by rw [show signExtend12 (40 : BitVec 12) = (40 : Word) from by native_decide]
            exact ValidMemRange.fetch hvalid 5 _ (by omega) (by rfl))
        (by rw [j1_u1_addr_eq_s0n1]; exact hv_u2)
        (by rw [show signExtend12 (48 : BitVec 12) = (48 : Word) from by native_decide]
            exact ValidMemRange.fetch hvalid 6 _ (by omega) (by rfl))
        (by rw [j1_u2_addr_eq_s0n1]; exact hv_u3)
        (by rw [show signExtend12 (56 : BitVec 12) = (56 : Word) from by native_decide]
            exact ValidMemRange.fetch hvalid 7 _ (by omega) (by rfl))
        (by rw [j1_u3_addr_eq_s0n1]; exact hv_u4)
        (by rw [j1_u4_addr_eq_s0n1]; exact hv_u5)
        (by rw [j1_q_addr_eq_s0n1]; exact hv_q1)
      dsimp only [] at hJ1
      simp only [j1_u0_addr_eq_s0n1, j1_q_addr_eq_s0n1, j1_u1_addr_eq_s0n1,
        j1_u2_addr_eq_s0n1, j1_u3_addr_eq_s0n1, j1_u4_addr_eq_s0n1,
      ...
      · -- j=1 loop-back: now execute j=0
        ...
        have hLB0 := divK_loop_body_n1_j0_spec sp
          ...
          base
          hv_j hv_n
          (by rw [j0_uhi_addr_eq_n1_s0]; exact hv_u1)
          (by rw [j0_ulo_addr_eq_n1_s0]; exact hv_u0)
          (by rw [j0_vtop_addr_eq_n1_s0]; exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
          hv_ret hv_d hv_dlo hv_scratch
          halign
          (by rw [show signExtend12 (32 : BitVec 12) = (32 : Word) from by native_decide]
              exact ValidMemRange.fetch hvalid 4 _ (by omega) (by rfl))
          (by rw [j0_u0_addr_eq_s0n1]; exact hv_u0)
          (by rw [show signExtend12 (40 : BitVec 12) = (40 : Word) from by native_decide]
              exact ValidMemRange.fetch hvalid 5 _ (by omega) (by rfl))
          (by rw [j0_u1_addr_eq_s0n1]; exact hv_u1)
          (by rw [show signExtend12 (48 : BitVec 12) = (48 : Word) from by native_decide]
              exact ValidMemRange.fetch hvalid 6 _ (by omega) (by rfl))
          (by rw [j0_u2_addr_eq_s0n1]; exact hv_u2)
          (by rw [show signExtend12 (56 : BitVec 12) = (56 : Word) from by native_decide]
              exact ValidMemRange.fetch hvalid 7 _ (by omega) (by rfl))
          (by rw [j0_u3_addr_eq_s0n1]; exact hv_u3)
          (by rw [j0_u4_addr_eq_s0n1]; exact hv_u4)
          (by rw [j0_q_addr_eq_s0n1]; exact hv_q0)
        dsimp only [] at hLB0
        simp only [j0_u0_addr_eq_s0n1, j0_q_addr_eq_s0n1, j0_u1_addr_eq_s0n1,
          j0_u2_addr_eq_s0n1, j0_u3_addr_eq_s0n1, j0_u4_addr_eq_s0n1,
...
-- ============================================================================
-- Full n=1 MOD spec (shift=0): base → base+1064
...
/-- Full n=1 MOD spec (b[3]=b[2]=b[1]=0, b[0]≠0, shift=0): base → base+1064.
    ...
    MOD returns u values (remainder) rather than q values. -/
theorem evm_mod_n1_shift0_full_spec (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11 : Word)
    (q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7 : Word)
    (n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0 : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
    ...
    cpsTriple base (base + 1064) (modCode base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ v5) ** (.x10 ↦ᵣ v10) ** (.x0 ↦ᵣ (0 : Word)) **
       ...
       ((sp + signExtend12 3968) ↦ₘ ret_mem) ** ((sp + signExtend12 3960) ↦ₘ d_mem) **
       ((sp + signExtend12 3952) ↦ₘ dlo_mem) ** ((sp + signExtend12 3944) ↦ₘ scratch_un0))
      (fun h => ∃ (u0out u1out u2out u3out x2out x1out x11out : Word)
        (u0v_i u1v_i u2v_i u3v_i u4out u5out u6out u7out : Word)
        (j_mem_out : Word)
        (retout dout dlout scout : Word),
        ((.x12 ↦ᵣ (sp + 32)) ** (.x5 ↦ᵣ u0out) ** (.x6 ↦ᵣ u1out) ** (.x7 ↦ᵣ u2out) **
         ...
         ((sp + signExtend12 4088) ↦ₘ u0v_i) **
         ((sp + signExtend12 4080) ↦ₘ u1v_i) **
         ((sp + signExtend12 4072) ↦ₘ u2v_i) **
         ((sp + signExtend12 4064) ↦ₘ u3v_i) **
         ((sp + signExtend12 4024) ↦ₘ u4out) **
  ...
  have hPLLB := evm_mod_n1_shift0_preloop_loopbody_spec sp base
    ...
    hbnz hb3z hb2z hb1z hshift_z hvalid halign
  ...
  -- Get shift=0 MOD epilogue spec (base+904 → base+1064)
  -- Since shift=0, no denormalization, b values pass through unchanged
  -- MOD epilogue reads u values from memory and writes them to output
  have hDE := evm_mod_shift0_epilogue_spec sp base
    u0v u1v u2v u3v ((clzResult b0).1)
  ...
  -- POST_LOOP_PRE = shift=0 MOD epilogue precondition (16 atoms)
  let POST_LOOP_PRE :=
    ...
    ((sp + 32) ↦ₘ b0) ** ((sp + 40) ↦ₘ b1) **
    ((sp + 48) ↦ₘ b2) ** ((sp + 56) ↦ₘ b3)
  -- LEFTOVER: q[] + u4-u7 + a[] + other scratch
  ...
  -- MOD epilogue output: x5=u0v, x6=u1v, x7=u2v, x10=u3v (u values, remainder)
  -- shift=0 so (clzResult b0).1=0, stored at 3992
  -- q values go in LEFTOVER slots at 4088/4080/4072/4064
  exact ⟨u0v, u1v, u2v, u3v, _, x1v, x11v,
    q0v, q1v, q2v, q3v, u4v, u5v, u6v, u7v, j_v,
    retv, dv, dlov, sunv,
    by xperm_hyp hPL⟩
