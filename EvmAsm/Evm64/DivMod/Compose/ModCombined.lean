/-
  EvmAsm.Evm64.DivMod.Compose.ModCombined

  Combined MOD full spec for the b!=0 case, unifying all n-cases (n=4,3,2,1)
  and both shift variants (shift=0 and shift!=0) into a single theorem.

  Each per-n spec has a different postcondition shape:
  - n=4: 1 q value (qv0), rest zero; 4 u outputs + u4out
  - n=3: 2 q values (qv0, qv1), rest zero; u0-u5 outputs
  - n=2: 3 q values (qv0, qv1, qv2), rest zero; u0-u6 outputs
  - n=1: 4 q values (q0v, q1v, q2v, q3v); all u outputs
  ...
  Key difference from DivCombined: the epilogue outputs remainder (u values)
  instead of quotient. Registers x5=u0out, x6=u1out, x7=u2out, x10=u3out
  and output memory sp+32..56 = u0out..u3out (denormalized remainder).
  The q values exist only in scratch memory (sp + signExtend12 4088..4064).
-/

import EvmAsm.Evm64.DivMod.Compose.ModN4Full
import EvmAsm.Evm64.DivMod.Compose.ModN4FullShift0
import EvmAsm.Evm64.DivMod.Compose.ModN3Full
import EvmAsm.Evm64.DivMod.Compose.ModN3FullShift0
import EvmAsm.Evm64.DivMod.Compose.ModN2Full
import EvmAsm.Evm64.DivMod.Compose.ModN2FullShift0
import EvmAsm.Evm64.DivMod.Compose.ModN1Full
import EvmAsm.Evm64.DivMod.Compose.ModN1FullShift0
...
-- ============================================================================
-- Combined b!=0 MOD full spec: base -> base+1064
-- ============================================================================

/-- Combined MOD full spec for b!=0: handles all n-cases (n=4,3,2,1) x shift variants.
...
theorem evm_mod_bnz_full_spec (sp base : Word)
  ...
  · -- b3 = 0
    ...
    · -- b3 = b2 = 0
      ...
      · -- n=1 case: b3=b2=b1=0
        ...
        · -- n=1 shift=0
          have h := evm_mod_n1_shift0_full_spec sp base
          ...
          exact cpsTriple_consequence _ _ _ _ _ _ _
            (fun h hp => hp)
            (fun h hq => by
              obtain ⟨u0out, u1out, u2out, u3out, x2out, x1out, x11out,
                u0v_i, u1v_i, u2v_i, u3v_i, u4out, u5out, u6out, u7out,
                j_mem_out, retout, dout, dlout, scout, hA⟩ := hq
              exact ⟨u0v_i, u1v_i, u2v_i, u3v_i, x2out, x1out, x11out,
                u0out, u1out, u2out, u3out, u4out,
                _, (1 : Word), j_mem_out, u5out, u6out, u7out,
                retout, dout, dlout, scout, by xperm_hyp hA⟩)
            h
        · -- n=1 shift!=0
          have h := evm_mod_n1_full_spec sp base
            a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
            q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
            n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
            hbnz hb3 hb2 hb1 hshift hvalid halign
      ...
      · -- n=2 case: b3=b2=0, b1!=0
        ...
        · -- n=2 shift=0
          have h := evm_mod_n2_shift0_full_spec sp base
            a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
            q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
            n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
            hbnz hb3 hb2 hb1 hshift hvalid halign
          ...
          exact cpsTriple_consequence _ _ _ _ _ _ _
            (fun h hp => hp)
            (fun h hq => by
              obtain ⟨qv0, qv1, qv2, x2out, x1out, x11out,
                u0out, u1out, u2out, u3out, u4out, u5out, u6out,
                j_mem_out, retout, dout, dlout, scout, hA⟩ := hq
              exact ⟨qv0, qv1, qv2, (0 : Word), x2out, x1out, x11out,
                u0out, u1out, u2out, u3out, u4out,
                _, (2 : Word), j_mem_out, u5out, u6out, (0 : Word),
                retout, dout, dlout, scout, by xperm_hyp hA⟩)
            h
        · -- n=2 shift!=0
          have h := evm_mod_n2_full_spec sp base
            a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
            q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
            n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
            hbnz hb3 hb2 hb1 hshift hvalid halign
    ...
    · -- n=3 case: b3=0, b2!=0
      ...
      · -- n=3 shift=0
        have h := evm_mod_n3_shift0_full_spec sp base
          a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
          q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
          n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
        ...
        exact cpsTriple_consequence _ _ _ _ _ _ _
          (fun h hp => hp)
          (fun h hq => by
            obtain ⟨qv0, qv1, x2out, x1out, x11out,
              u0out, u1out, u2out, u3out, u4out, u5out,
              j_mem_out, retout, dout, dlout, scout, hA⟩ := hq
            exact ⟨qv0, qv1, (0 : Word), (0 : Word), x2out, x1out, x11out,
              u0out, u1out, u2out, u3out, u4out,
              _, (3 : Word), j_mem_out, u5out, (0 : Word), (0 : Word),
              retout, dout, dlout, scout, by xperm_hyp hA⟩)
          h
      · -- n=3 shift!=0
        have h := evm_mod_n3_full_spec sp base
          a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
          q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
          n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
          hbnz hb3 hb2 hshift hvalid halign
  ...
  · -- n=4 case: b3!=0
    ...
    · -- n=4 shift=0
      have h := evm_mod_n4_shift0_full_spec sp base
        a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
        q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
        n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
      ...
      exact cpsTriple_consequence _ _ _ _ _ _ _
        ...
        h
    · -- n=4 shift!=0
      have h := evm_mod_n4_full_spec sp base
        a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11
        q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7
        n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0
        hbnz hb3 hshift hvalid halign
