/-
  EvmAsm.Evm64.DivMod.Compose.ModN4FullShift0

  End-to-end composition for n=4 MOD (256-bit % 256-bit, b[3]≠0, shift=0).
  Composes: pre-loop (base→base+448) + loop body j=0 (base+448→base+904) +
            post-loop shift=0 (base+904→base+1064).
-/
...
-- (Same as in ModN4Full.lean, re-declared as private)
-- ============================================================================
...
/-- Pre-loop + single-iteration loop body for n=4 MOD (shift=0).
    Composes base→base+448 (copy path) with base+448→base+904 (loop body j=0).
    Postcondition uses loopBodyPostN4 with existentials for computed values. -/
theorem evm_mod_n4_shift0_preloop_loopbody_spec (sp base : Word)
  ...
  have hPL := evm_mod_n4_shift0_to_loopSetup_spec sp base a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10
    q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7 n_mem shift_mem
    hbnz hb3nz hshift_z hvalid hv_q0 hv_q1 hv_q2 hv_q3
  ...
  have hLB := divK_loop_body_n4_j0_spec sp j_old (4 : Word) ((clzResult b3).1)
    ((clzResult b3).2 >>> (63 : Nat))
    b3 v11 (signExtend12 (0 : BitVec 12) - (clzResult b3).1)
  ...
  -- Rewrite x1 in pre-loop postcondition
  rw [se12_4_sub_4] at hPLf
  -- Expand let-bindings and canonicalize addresses
  dsimp only [] at hLB
  ...
  -- Lift loop body from sharedDivModCode to modCode
  have hLB := cpsTriple_extend_code (sharedDivModCode_sub_modCode base) hLB
  -- Frame loop body
...
-- ============================================================================
-- Full end-to-end: base → base+1064 for n=4 MOD (shift=0)
-- ============================================================================

set_option maxRecDepth 4096 in
set_option maxHeartbeats 6400000 in
/-- Full n=4 MOD spec (shift=0): base → base+1064.
    Composes pre-loop + loop body (base→base+904) with
    shift=0 epilogue (base+904→base+1064). -/
theorem evm_mod_n4_shift0_full_spec (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11 : Word)
    (q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7 : Word)
    (n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0 : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
  ...
  have hPLLB := evm_mod_n4_shift0_preloop_loopbody_spec sp base
    ...
    hbnz hb3nz hshift_z hvalid halign
  ...
  -- POST_LOOP_PRE for shift=0 MOD: registers + shift_mem + u[] + output memory (16 atoms)
  let POST_LOOP_PRE :=
