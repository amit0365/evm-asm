...
/-- DIV full n=3 path (shift ≠ 0): b[3]=b[2]=0, b[1]≠0, shift=clzResult(b1).1≠0.
    base → base+448. -/
theorem evm_div_n2_to_loopSetup_spec (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 n_mem shift_mem : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
    (hb3z : b3 = 0) (hb2z : b2 = 0) (hb1nz : b1 ≠ 0)
...
/-- DIV full n=3 path (shift = 0): b[3]=b[2]=0, b[1]≠0, clzResult(b1).1=0.
...
theorem evm_div_n2_shift0_to_loopSetup_spec (sp base : Word)
    ...
    cpsTriple base (base + loopBodyOff) (divCode base)
      ...
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ (2 : Word)) ** (.x10 ↦ᵣ b3) ** (.x0 ↦ᵣ (0 : Word)) **
       ...
       (.x1 ↦ᵣ signExtend12 (4 : BitVec 12) - (2 : Word)) **
       ((sp + 0) ↦ₘ a0) ** ((sp + 8) ↦ₘ a1) **
       ((sp + 16) ↦ₘ a2) ** ((sp + 24) ↦ₘ a3) **
       ((sp + 32) ↦ₘ b0) ** ((sp + 40) ↦ₘ b1) **
       ((sp + 48) ↦ₘ b2) ** ((sp + 56) ↦ₘ b3) **
  ...
  have hCopy := divK_copyAU_full_spec sp a0 a1 a2 a3
    u0Old u1Old u2Old u3Old u4Old ((clzResult b1).2) base
