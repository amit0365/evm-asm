/-
  EvmAsm.Evm64.DivMod.Compose.ModN2FullShift0

  End-to-end composition for n=2 MOD (b[3]=0, b[2]=0, b[1]≠0, shift=0).
...
-- ============================================================================
-- Address simplification lemmas for j=0 (n=2, shift=0)
-- ============================================================================
...
-- ============================================================================
-- Address simplification lemmas for j=1 (n=2, shift=0)
-- ============================================================================
...
private theorem j1_uhi_addr_eq_s0 (sp : Word) :
...
private theorem j1_ulo_addr_eq_s0 (sp : Word) :
...
-- ============================================================================
-- Address simplification lemmas for j=2 (n=2, shift=0)
-- ============================================================================
...
private theorem j2_uhi_addr_eq_s0 (sp : Word) :
...
private theorem j2_ulo_addr_eq_s0 (sp : Word) :
...
/-- x1 in pre-loop for n=2 shift=0: signExtend12 4 - 2 = 2 -/
...
/-- Pre-loop + three-iteration loop body for n=2 MOD (shift=0).
    ...
    at base+448: j=2 (cpsBranch, loops back) then j=1 (cpsBranch, loops back) then
    j=0 (cpsTriple, exits to base+904).
...
theorem evm_mod_n2_shift0_preloop_loopbody_spec (sp base : Word)
  ...
  have hPL := evm_mod_n2_shift0_to_loopSetup_spec sp base a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10
  ...
  -- The j=2 window: u[0] at sp+SE12(4040)=a2, u[1] at sp+SE12(4032)=a3,
  ...
  have hJ2 := divK_loop_body_n2_combined_spec
    sp (2 : Word) j_old (2 : Word) (clzResult b1).1
    ((clzResult b1).2 >>> (63 : Nat)) b3 v11
    (signExtend12 (0 : BitVec 12) - (clzResult b1).1)
    b0 b1 b2 b3 a2 a3 (0 : Word) (0 : Word) (0 : Word) (0 : Word)
    ret_mem d_mem dlo_mem scratch_un0
    base
    hv_j hv_n
    (by rw [j2_uhi_addr_eq_s0]; exact hv_u4)
    (by rw [j2_ulo_addr_eq_s0]; exact hv_u3)
  ...
  obtain ⟨k2, s2, hstep2, hcase⟩ := hJ2f F hF s1 hcr1
  ...
  · -- Loop-back case: j=2 looped back to base+448, now execute j=1 then j=0
    ...
    have hJ1 := divK_loop_body_n2_combined_spec sp
      ...
      base
      hv_j hv_n
      (by rw [j1_uhi_addr_eq_s0]; exact hv_u3)
      (by rw [j1_ulo_addr_eq_s0]; exact hv_u2)
      (by rw [j0_vtop_addr_eq_n2_s0]; exact ValidMemRange.fetch hvalid 5 _ (by omega) (by rfl))
      hv_ret hv_d hv_dlo hv_scratch
      halign
      (by rw [show signExtend12 (32 : BitVec 12) = (32 : Word) from by native_decide]
    ...
    -- Frame j=1 with cells it doesn't touch
...
-- ============================================================================
-- Full n=2 MOD spec (shift=0): base → base+1064
-- Composes preloop+loopbody (base→base+904) with shift=0 epilogue (base+904→base+1064)
-- ============================================================================

set_option maxRecDepth 4096 in
set_option maxHeartbeats 6400000 in
/-- Full n=2 MOD spec (b[3]=0, b[2]=0, b[1]≠0, shift=0): base → base+1064.
...
theorem evm_mod_n2_shift0_full_spec (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11 : Word)
    (q0 q1 q2 q3 u0_old u1_old u2_old u3_old u4_old u5 u6 u7 : Word)
    (n_mem shift_mem j_old ret_mem d_mem dlo_mem scratch_un0 : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
  ...
  have hPLLB := evm_mod_n2_shift0_preloop_loopbody_spec sp base
