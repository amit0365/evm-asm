/-
  EvmAsm.Evm64.DivMod.Compose.DivN2Full

  End-to-end composition for n=2 DIV (b[3]=b[2]=0, b[1]≠0, shift≠0).
...
private theorem j1_uhi_addr_eq_n2 (sp : Word) :
...
private theorem j1_ulo_addr_eq_n2 (sp : Word) :
...
private theorem j2_uhi_addr_eq_n2 (sp : Word) :
...
private theorem j2_ulo_addr_eq_n2 (sp : Word) :
...
theorem evm_div_n2_preloop_loopbody_spec (sp base : Word)
  ...
  have hPL := evm_div_n2_to_loopSetup_spec sp base a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10
  ...
  have hJ2 := divK_loop_body_n2_combined_spec
    ...
    base
    hv_j hv_n
    (by rw [j2_uhi_addr_eq_n2]; exact hv_u4)
    (by rw [j2_ulo_addr_eq_n2]; exact hv_u3)
  ...
  -- Frame j=2 with cells not in its precondition
  ...
  · -- Loop-back case: j=2 looped back to base+448, now execute j=1 then j=0
    ...
    -- Memory j_old=(2:Word) (stored by j=2), n_mem=(2:Word)
    ...
    have hJ1 := divK_loop_body_n2_combined_spec
      ...
      base
      hv_j hv_n
      (by rw [j1_uhi_addr_eq_n2]; exact hv_u3)
      (by rw [j1_ulo_addr_eq_n2]; exact hv_u2)
      (by rw [j0_vtop_addr_eq_n2]; exact ValidMemRange.fetch hvalid 5 _ (by omega) (by rfl))
      hv_ret hv_d hv_dlo hv_scratch
      halign
      (by rw [show signExtend12 (32 : BitVec 12) = (32 : Word) from by native_decide]
    ...
    -- Frame j=1 with cells not in its precondition (includes j=2 outputs)
    ...
    · -- j=1 loop-back: now execute j=0
      ...
      -- Memory j_old=(1:Word), n_mem=(2:Word)
...
theorem evm_div_n2_full_spec (sp base : Word)
  ...
  have hPLLB := evm_div_n2_preloop_loopbody_spec sp base
  ...
  let POST_LOOP_PRE :=
    ...
    ((sp + 32) ↦ₘ b0') ** ((sp + 40) ↦ₘ b1') **
    ((sp + 48) ↦ₘ b2') ** ((sp + 56) ↦ₘ b3')
  -- LEFTOVER = atoms carried through: x1, x11, j, n, u4, u5, u6, a0-a3, u7(=0), ret, d, dlo, scratch
