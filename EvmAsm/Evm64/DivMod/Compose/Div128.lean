...
theorem div128_spec (sp ret_addr d u_lo u_hi : Word) (base : Word)
    ...
    let rhat' := if BitVec.ult rhatUn1 qDlo then rhatc + d_hi else rhatc
    ...
    cpsTriple (base + div128Off) ret_addr (sharedDivModCode base)
      ...
      (-- Postcondition: x11=quotient, all regs/mem updated
       ...
       (.x5 ↦ᵣ q0') ** (.x7 ↦ᵣ q0Dlo) **
       (.x6 ↦ᵣ d_hi) ** (.x1 ↦ᵣ rhat2Un0) ** (.x11 ↦ᵣ q) **
       (.x0 ↦ᵣ (0 : Word)) **
  ...
  intro d_hi d_lo un1 un0 q1 rhat hi1 q1c rhatc qDlo rhatUn1 q1' rhat' cu_rhat_un1 cu_q1_dlo un21 q0 rhat2 hi2 q0c rhat2c q0Dlo rhat2Un0 q0' q
  -- ================================================================
  ...
  -- ================================================================
  have hcu := divK_div128_compute_un21_spec sp q1' rhat' un1 rhatUn1 qDlo d_lo
    (base + 1172)
  ...
  have hendf := cpsTriple_frameR
    ((.x7 ↦ᵣ q0Dlo) ** (.x6 ↦ᵣ d_hi) ** (.x1 ↦ᵣ rhat2Un0) **
     (.x0 ↦ᵣ (0 : Word)) **
