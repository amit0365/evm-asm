...
theorem evm_div_phaseAB_n1_clz_spec (sp base : Word)
  ...
  have hB := evm_div_phaseB_n1_spec sp base b0 b1 b2 b3
    ...
    hb3z hb2z hb1z hvalid hv_q0 hv_q1 hv_q2 hv_q3 hv_u5 hv_u6 hv_u7 hv_n
...
/-- DIV full n=3 path (shift ≠ 0): b[3]=b[2]=b[1]=0, shift=clzResult(b0).1≠0.
    base → base+448. -/
theorem evm_div_n1_to_loopSetup_spec (sp base : Word)
    ...
    cpsTriple base (base + 448) (divCode base)
      ...
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ (1 : Word)) ** (.x10 ↦ᵣ (a0 >>> (anti_shift.toNat % 64))) **
       (.x0 ↦ᵣ (0 : Word)) **
  ...
  have hABCLZ := evm_div_phaseAB_n1_clz_spec sp base b0 b1 b2 b3 v5 v6 v7 v10
    q0 q1 q2 q3 u5 u6 u7 n_mem hbnz hb3z hb2z hb1z hvalid
...
/-- DIV full n=3 path (shift = 0): b[3]=b[2]=b[1]=0, clzResult(b0).1=0.
...
theorem evm_div_n1_shift0_to_loopSetup_spec (sp base : Word)
  ...
  have hC2 := divK_phaseC2_taken_spec sp ((clzResult b0).1)
    ((clzResult b0).2 >>> (63 : Nat)) shift_mem base hv_shift hshift_z
  have hC2f := cpsTriple_frame_left _ _ _ _ _
    ((.x5 ↦ᵣ (clzResult b0).2) ** (.x10 ↦ᵣ b3) **
     (.x7 ↦ᵣ (clzResult b0).2 >>> (63 : Nat)) **
     (.x1 ↦ᵣ signExtend12 (4 : BitVec 12) - (4 : Word)) **
