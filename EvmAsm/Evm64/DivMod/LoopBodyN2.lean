...
private theorem vtop_eq_v1_n2 (sp : Word) :
  ...
  simp only [show (3 : BitVec 6).toNat = 3 from by native_decide,
    ...
    show signExtend12 (40 : BitVec 12) = (40 : Word) from by native_decide]
  bv_omega
...
theorem divK_loop_body_n2_max_skip_spec
    ...
    (hv_uhi : isValidDwordAccess (sp + signExtend12 4056 - (j + (2 : Word)) <<< (3 : BitVec 6).toNat) = true)
    (hv_ulo : isValidDwordAccess ((sp + signExtend12 4056 - (j + (2 : Word)) <<< (3 : BitVec 6).toNat) + 8) = true)
...
/-- Postcondition for one loop iteration at n=2.
...
/-- Combined loop body for n=2: one iteration at base+448.
    cpsBranch to base+448 (loop back) or base+904 (exit).
    No overlapping cells — precondition uses only u_base-relative addresses.
    All 4 paths (max/call × skip/addback) handled internally. -/
theorem divK_loop_body_n2_combined_spec
    (sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
     v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old : Word)
    (ret_mem d_mem dlo_mem scratch_un0 : Word)
    (base : Word)
  ...
  · -- BLTU taken: div128 call path
    ...
    · -- Call + skip (borrow = 0)
      have HS := divK_loop_body_n2_call_skip_spec sp j j_old v5_old v6_old v7_old v10_old
        v11_old v2_old v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old
    ...
    · -- Call + addback (borrow ≠ 0)
      ...
      let aun4 := u4_new + aco3
      let q_hat' := q_hat + signExtend12 4095
      have HA := divK_loop_body_n2_call_addback_spec sp j j_old v5_old v6_old v7_old v10_old
        v11_old v2_old v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old
        ret_mem d_mem dlo_mem scratch_un0 base
        hv_j hv_n1 hv_uhi hv_ulo hv_vtop hv_ret hv_d hv_dlo hv_scratch_un0 halign
        hv_v0 hv_u0 hv_v1 hv_u1 hv_v2 hv_u2 hv_v3 hv_u3 hv_u4 hv_q hbltu
  ...
  · -- BLTU not taken: max path
    ...
    by_cases hborrow : borrow = (0 : Word)
    · -- Max + skip (borrow = 0)
      have HS := divK_loop_body_n2_max_skip_spec sp j j_old v5_old v6_old v7_old v10_old
        v11_old v2_old v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old base
    ...
    · -- Max + addback (borrow ≠ 0)
      ...
      let aun4 := u4_new + aco3
      let q_hat' := q_hat + signExtend12 4095
      have HA := divK_loop_body_n2_max_addback_spec sp j j_old v5_old v6_old v7_old v10_old
        v11_old v2_old v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old base
        hv_j hv_n1 hv_uhi hv_ulo hv_vtop
        hv_v0 hv_u0 hv_v1 hv_u1 hv_v2 hv_u2 hv_v3 hv_u3 hv_u4 hv_q hbltu
      dsimp only [] at HA
...
-- ============================================================================
-- Section 18n2: Single-iteration cpsTriple for n=2 at j=0
-- Composes trial + mulsub_correction + store_loop_j0 → cpsTriple base+448 → base+904
-- ============================================================================

set_option maxRecDepth 8192 in
set_option maxHeartbeats 12800000 in
/-- Single-iteration loop body for n=2 at j=0: cpsTriple base+448 → base+904.
    Since j=0 and BGE(j'=-1) is not taken, this always exits the loop.
    All 4 paths (max/call × skip/addback) handled internally. -/
theorem divK_loop_body_n2_j0_spec
    ...
    (hv_j : isValidDwordAccess (sp + signExtend12 3976) = true)
    (hv_n1 : isValidDwordAccess (sp + signExtend12 3984) = true)
    (hv_uhi : isValidDwordAccess (sp + signExtend12 4056 - ((0 : Word) + (2 : Word)) <<< (3 : BitVec 6).toNat) = true)
    (hv_ulo : isValidDwordAccess ((sp + signExtend12 4056 - ((0 : Word) + (2 : Word)) <<< (3 : BitVec 6).toNat) + 8) = true)
    (hv_vtop : isValidDwordAccess (sp + ((2 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat + signExtend12 32) = true)
    (hv_ret : isValidDwordAccess (sp + signExtend12 3968) = true)
    (hv_d   : isValidDwordAccess (sp + signExtend12 3960) = true)
    (hv_dlo : isValidDwordAccess (sp + signExtend12 3952) = true)
  ...
  by_cases hbltu : BitVec.ult u2 v1
  · -- BLTU taken: div128 call path
    -- Compute div128 + mulsub intermediates (same as combined_spec)
    let d_hi := v1 >>> (32 : BitVec 6).toNat
    let d_lo := (v1 <<< (32 : BitVec 6).toNat) >>> (32 : BitVec 6).toNat
    let div_un1 := u1 >>> (32 : BitVec 6).toNat
    let div_un0 := (u1 <<< (32 : BitVec 6).toNat) >>> (32 : BitVec 6).toNat
