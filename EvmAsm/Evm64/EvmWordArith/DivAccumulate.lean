...
theorem iter_accumulate_1 {u_val v_val q0Nat r_val : Nat}
    (h0 : u_val = q0Nat * v_val + r_val) :
    u_val = q0Nat * v_val + r_val := h0
...
/-- 2 iterations (n=3 case): digits q1 (high) and q0 (low).
    ...
    full quotient. -/
theorem iter_accumulate_2 {u_val v_val q1Nat q0Nat mid_val r_val : Nat}
    (h1 : u_val = q1Nat * v_val * 2^64 + mid_val)
    (h0 : mid_val = q0Nat * v_val + r_val) :
    u_val = (q1Nat * 2^64 + q0Nat) * v_val + r_val := by
  nlinarith

/-- 3 iterations (n=2 case): digits q2, q1, q0. -/
theorem iter_accumulate_3 {u_val v_val q2Nat q1Nat q0Nat mid2_val mid1_val r_val : Nat}
    (h2 : u_val = q2Nat * v_val * 2^128 + mid2_val)
    (h1 : mid2_val = q1Nat * v_val * 2^64 + mid1_val)
    (h0 : mid1_val = q0Nat * v_val + r_val) :
    u_val = (q2Nat * 2^128 + q1Nat * 2^64 + q0Nat) * v_val + r_val := by
  nlinarith

/-- 4 iterations (n=1 case): digits q3, q2, q1, q0. -/
theorem iter_accumulate_4
    {u_val v_val q3Nat q2Nat q1Nat q0Nat mid3_val mid2_val mid1_val r_val : Nat}
    (h3 : u_val = q3Nat * v_val * 2^192 + mid3_val)
    (h2 : mid3_val = q2Nat * v_val * 2^128 + mid2_val)
    (h1 : mid2_val = q1Nat * v_val * 2^64 + mid1_val)
    (h0 : mid1_val = q0Nat * v_val + r_val) :
    u_val = (q3Nat * 2^192 + q2Nat * 2^128 + q1Nat * 2^64 + q0Nat) * v_val + r_val := by
  nlinarith
