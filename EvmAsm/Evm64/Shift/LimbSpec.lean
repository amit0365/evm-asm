...
theorem shr_merge_limb_spec (src_off next_off dst_off : BitVec 12)
    (sp src next dstOld v5 v10 bit_shift anti_shift mask : Word) (base : Word) :
  ...
  have SD_ := sd_spec_gen .x12 .x5 sp ((src >>> (bit_shift.toNat % 64)) ||| ((next <<< (anti_shift.toNat % 64)) &&& mask)) dstOld dst_off (base + 24)
